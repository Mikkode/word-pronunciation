...
export default function Card1({
  ...
  return (
    ...
    <div className={`relative aspect-square ${className}`} onClick={onClick}>
      ...
      <div
        className="absolute inset-0 border-2 border-black z-0 transform rotate-[-6deg] rounded-2xl"
        style={{ backgroundColor: color1 }}
      ></div>

      {/* Deuxième feuille (rotation droite) - contient maintenant le contenu principal */}
      <div
        className="absolute inset-0 border-2 border-black z-10 transform rotate-[3deg] overflow-hidden rounded-2xl"
        style={{ backgroundColor: "white" }}
      >
        {/* Contenu principal - sans bordure, juste un div intérieur pour garder l'image droite */}
        <div
          className="absolute inset-0 z-20 transform rotate-[-4deg] overflow-hidden flex items-center justify-center"
